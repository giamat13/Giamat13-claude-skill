...
This plugin ships a `SessionStart` hook that prints the rules straight into context at the start of every session, so they are there whether or not the model would have reached for the skill. The skill itself is still included, so `/better-tool-use` works on demand.
...
```
/plugin marketplace add giamat13/Giamat13-claude-skill
/plugin install better-tool-use@Giamat13
```
...
```
.claude-plugin/marketplace.json        marketplace entry (name: Giamat13)
better-tool-use/
  .claude-plugin/plugin.json           plugin manifest (name: better-tool-use)
  hooks/hooks.json                     SessionStart hook
  hooks/inject.mjs                     prints the SKILL.md body
  skills/better-tool-use/SKILL.md      the rules (single source of truth)
```
